...
可以看到，收敛速度还是挺慢的，那么有没有速度更快的求平方根算法呢？这个时候Newton法就有话要说了：当然有了。
...
其中$f$是二阶连续可微的，且存在唯一 $x^\*$ 使得 $f(x^\*)=0$ ，则给定初始点 $x^0$ ,Newton法的迭代格式如下：
...
可以看到，Newton法用了5步就找到了最优解，吊打二分法一条街。Newton法一般在目标函数单调时效果比较好，当函数有多个零点或者多重零点时，Newton法的收敛速率会比较差，我们在此不做具体讨论。关于Newton法的收敛速率可以参考[2]。
...
那么 $s = 0_{(2)}$  ,  $ e+127=129=10000001_{(2)} $ ,以及

$$
f=0.57562494277954101563=0.10010011010111000010100_{(2)}
$$
