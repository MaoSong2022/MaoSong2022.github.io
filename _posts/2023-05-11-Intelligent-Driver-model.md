...
# 模型特征
自车的加速度$a$可以被分成自由项和交互项：

$$
\dot{v}_{\alpha}^{free}=a\left[1-\left(\frac{v_\alpha}{v_0}\right)^\delta\right],\quad \dot{v}_{\alpha}^{init}=-a\left(\frac{s^*(v_\alpha,\Delta v_\alpha)}{s_\alpha}\right)^2=-a\left(\frac{s_0+v_\alpha T}{s_\alpha}+\frac{v_\alpha\Delta v_\alpha}{2\sqrt{ab}s_\alpha}\right)^2
$$
