...
> 除了虚函数信息外，vtable里还包含了RTTI的信息，关于RTTI可以参考之前的[博客](obsidian://open?vault=MaoSong2022.github.io&file=_posts%2F2023-04-23-RTTI)
{: .prompt-info }
...
- 调用虚函数的流程？
> 首先通过位于栈区的指向类的实例的指针找到位于堆区的实例的地址，然后通过实例内存开头的vptr找到位于只读数据区`.rodata`里的vtable，再根据vtable里的index找到对应的函数，最后跳转到代码区`.text`中对应的虚函数代码。
- 虚函数越多越好吗？ 并不是，虚函数增加会增加vtable的size，在vtable里查找会更加耗时。实际上，现在已经在探讨如何避免使用虚函数表来实现多态了，比如CRTP (Curiously Recurring Template Pattern).
