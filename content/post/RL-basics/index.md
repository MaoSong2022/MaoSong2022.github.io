...
> 注意
> 这里的期望 $\mathbb{E}_{s_0\sim p_0}^\pi$ 对应的随机变量为 $a_t\sim\pi(\cdot\mid s_t)$, $(r_t,s_{t+1})\sim p(\cdot,\cdot\mid s_t,a_t)$.

## Connection with LLM

我们将 LLM 记为 $p_{\theta}$, 这里 $\theta$ 是 LLM 的参数，将 LLM 的输入记为 $x$, 这里 $x$ 一般就是一个 prompt, LLM 每次生成一个新的 token, 我们记为 $y_i$, 表达式为

$$
y_i \sim p(y\mid y_{<i}, x)
$$
...
| action        | a single output token $y_i$           |
| state         | prompt and output token $(y_{<i}, x)$ |
| reward        | verifier or reward model              |
