...
import gpt_oss_architecture from "../../../assets/image/gpt-oss-architecture.png";
import gpt_oss_precision_format from "../../../assets/image/gpt-oss-precision-format.png";
import gpt_oss_chat_template from "../../../assets/image/gpt-oss-chat-template.png";
import gpt_oss_performance from "../../../assets/image/gpt-oss-performance.png";

## GPT-oss

openAI 发布了 gpt-oss [@GPT-oss] 大语言模型，包含 120B-A5.1B 以及 20.9B-A3.6B 两个 size, 作者强调了模型的 instruction following, tool use, 以及 adaptive thinking 能力

### Architecture

gpt-oss 系列是一个基于 MoE transformer 架构的 LLM.
架构中交替使用 sliding window attention 和 full attention, sliding window size 为 128 token, 架构图如下所示

<Image src={gpt_oss_architecture} alt="gpt-oss-architecture" />
...
大多数模型使用的基于 SwiGLU [@SwiGLU] 的 MLP 定义如下
...
作者使用了 [attention sink](/p/notes-on-streamingllm/) 来避免 window attention 在超过 kv cache size 之后，表现大幅度下降的问题。

### Quantization

为了降低模型的内存占用量，作者使用了 PTQ 来训练 MoE 的权重，使用的精度为 MXFP4, 这样每个参数由 4.25 bits 来表示。最终，模型的参数存储格式如下：

<Image src={gpt_oss_precision_format} alt="precision format of gpt-oss" />
...
预训练细节不多，主要是使用了 flash attention [@FlashAttention] 进行加速计算，使用了 Triton 进行了 kernel 的优化，gpt-oss-120b 训练了 120M H100-hours, gpt-oss-20B 的训练时间是 gpt-oss-120B 的十分之一左右
...
<Image src={gpt_oss_chat_template} alt="gpt-oss-chat-template" />

### Infra

GPT-oss 主要使用了如下 kernels:

1. lIGER RMSNorm
2. Megablocks MoE kernels
3. FlashAttention 3
4. MXFP4 triton kernels

kernel 的表现如下表所示 (256 tokens, 64 threads)

| Strategy    | Batch size | Time   |
| ----------- | ---------- | ------ |
| w/o kernels | 32         | 58.2s  |
|             | 64         | 113.5s |
|             | 128        | 224.0s |
| w kernels   | 32         | 11.9s  |
|             | 64         | 12.6s  |
...
<Image src={gpt_oss_performance} alt="performance of gpt-oss" />
