...
为了解决 MHA 的内存占用过高问题，已有的工作如 [MQA](/p/notes-on-mqa/), [GQA](/p/notes-on-gqa/) 等通过共享 key, value projection 来降低 KV cache size. 而 [DeepSeek-V3](/p/notes-on-deepseek-v3/) 提出的 MLA 则是通过对 key, value projection 进行 low-rank compression, 然后只存储 latents 的方法来降低 KV cache size.
