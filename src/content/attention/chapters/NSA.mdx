...
现有的大模型主要是基于 Transformer 提出的 softmax attention, 其主要问题在于随上下文长度增加，其 latency 也上升更快。
理论估计，对于 64k 上下文长度的输出，softmax attention 部分的计算占 $70\%\sim80\%$ 的 latency.
...
为了解决这两个问题，作者就提出了 natively trainable sparse attention (NSA) [@NSA]. NSA 通过将 key 和 value 分割为不同的 block, 然后基于三种 path: compressed coarse-grained tokens, selectively retrained fine-grained tokens 以及 sliding windows for local contextual information 来进行处理和过滤。NSA 提出了两点观点改进：
...
![Overview of NSA architecture](/posts/notes-on-nsa/NSA-architecture.png)
...
对于 [GQA](/p/notes-on-gqa/) 和 [MQA](/p/notes-on-mqa/), 由于其 KV-cache 在 heads 之间共享，因此我们必须保证不同 heads 之间的 consistency, 因此作者提出了 shared importance score 如下：
...
接下来是针对硬件设计进行的优化。由于 flash attention 2 [@FlashAttention2] 对 compression attention 以及 sliding window attention 已经支持的比较好，作者这里介绍了如何针对 selection attention 进行优化。

### Experiments

作者构建了一个 27B-A3B 的 MoE 模型，attention 基于 GQA, MoE 基于 [DeepSeekMoE](/p/notes-on-deepseekmoe/). 模型配置如下表所示
...
模型先在 8K 的上下文长度下使用 270B token 进行预训练，接下来在使用 [YARN](/p/notes-on-yarn/) 将模型上下文通过 continual pre-training 以及 SFT 扩展到 32K. 训练过程的损失如下图所示

![Training loss of NSA](/posts/notes-on-nsa/NSA-training-loss.png)
...
![Performance of NSA on general benchmarks](/posts/notes-on-nsa/NSA-general-performance.png)

接下来是 NSA 在 LongBench 上的表现：

![Performance of NSA on LongBench](/posts/notes-on-nsa/NSA-long-context-performance.png)

作者还使用了 [DeepSeek-R1](/p/notes-on-deepseek-r1/) 中的知识蒸馏方法，结果如下表所示
...
接下来，作者分析了 NSA 的性质。作者首先对比了 NSA 和 flash attention 2 [@FlashAttention2] 的训练速度，结果如下图所示

![Performance comparison between NSA and flash attention 2](/posts/notes-on-nsa/NSA-training-speed-performance.png)
