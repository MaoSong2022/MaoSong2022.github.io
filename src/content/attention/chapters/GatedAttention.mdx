...
在本文中，作者通过在 attention 中加入 gating 机制来探索 gating 对模型表现和训练稳定性的影响 [@GatedAttention]。尽管 gating 并没有降低 attention 计算复杂度，但是 gating 提出了一个新的视角，即 sparity 与 attention sink 和 massive activation 息息相关，这为后面 sparse attention 的研究提供了 Insight.
...
gating 早在 LSTM 和 GRU 使其就得到了广泛的运用，在 transformer 之后，相关的现行注意力也有应用，比如 [MiniMax-01](/p/notes-on-minimax-01/) 所使用的 Lightning Attention 等，但是这些工作没有系统性探究 gating 背后的机制。

第二部分是 attention sink, attention sink 现象由 [StreamingLLM](/p/notes-on-streamingllm/) 提出， 即模型会将相当一部分注意力权重方开始开始的几个 token 上。而本文提出的 gating 机制可以缓解 attention sink 现象。
...
这里 $\sigma$ 是激活函数，$W_\theta$ 是激活函数的可学习参数，我们可以将其理解为一个 linear layer, 即当前模块的输出取决于输入 hidden sates 经过一个线性层和激活层之后的结果，相似的做法还有 [MoE](/p/moe-tutorial/) 中的 gating layer, [NSA](/p/notes-on-nsa/) 中的 gating layer 等。对应的示意图如下所示

![Positions of different gating methods](/posts/notes-on-gated-attention/Gated-attention-position.png)
...
![Performance of different gating variants](/posts/notes-on-gated-attention/Gated-attention-gating-variant-performance.png)
...
![Performance of dense models with Gated attention](/posts/notes-on-gated-attention/Gated-attention-dense-model-performance.png)

结论验证了 gating 机制可以有效提高模型的表现。作者还发现使用 gating 之后，模型的训练也更加稳定，训练的损失变化曲线如下图所示

![training loss curve of gated attention](/posts/notes-on-gated-attention/Gated-attention-training-loss-curve.png)
...
也就是说，$W_K$ 和 $W_O$ 可以吸收到一起，由于 $W_V^j\in\mathbb{R}^{d\times d_k}$, $W_O^k\in\mathbb{R}^{d_k\times d}$, 从而 $\mathrm{rank}(W_V^jW_O^k)\leq \max(\mathrm{rank}(W_V^j), \mathrm{rank}(W_O^k))\leq d_k$.  对于 [GQA](/p/notes-on-gqa/) 和 [MQA](/p/notes-on-mqa/), 最终的有效秩会进一步降低。

而使用本文提到的 G1 和 G2 gating 策略之后，我们相当于是通过非线性机制提高了上面的秩，进而解决了 softmax attention 表达能力不足的问题, 实际上，StepFun 的 [MFA](/p/notes-on-mfa/) 也是类似的思想。下面是 G1 和 G2 做的改进：
...
![Performance of different non-linearity variants](/posts/notes-on-gated-attention/Gated-attention-non-linearity-performance.png)

可以看到，不同的 non-linearity 方法对模型表现都有提升，这验证了矩阵秩会影响模型表达能力的分析。

接下来，作者探究了 gating 机制对 attention score distribution 的影响，结果如下图所示

![attention score distribution of different methods](/posts/notes-on-gated-attention/Gated-attention-score-distribution.png)
...
![Visualization of attention sink](/posts/notes-on-gated-attention/Gated-attention-sink-visualization.png)
...
最后，作者探究了以下 gating 机制的上下文扩展能力，作者在已有的模型上基于 32k 上下文长度使用了 80B token 进行 continue pre-training, 然后使用 [YARN](/p/notes-on-yarn/) 将模型上下文长度扩展到了 128K。 测试的结果如下图所示
