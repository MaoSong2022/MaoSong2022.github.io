...
已有的工作有 MQA [@MQA], 也就是我们把多个 head 的 key layer 以及 value layer 压缩成一个，这样对于 $h$ 个 head 的 attention，我们有 $h$ 个 query layer，$1$ 个 key layer 以及 1 个 value layer. 但是 MQA 的问题在于其会导致性能下降，而且训练过程会不稳定。

因此，在本文中作者就提出了Group Query Attention (GQA) [@GQA]，一个在保持模型性能的同时，提高计算效率的注意力机制，作者还介绍了如何将一个 MHA 模型转化为一个一个 MQA 模型。
