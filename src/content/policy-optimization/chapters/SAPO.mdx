---
title: Notes on SAPO
description: Qwen 在 25 年 11 月提出了 SAPO，一个通过温度控制的 soft gate 以及非对称温度来解决 hard-clpping
  存在的问题，从而提高 RL 训练的稳定性以及效率
authors:
- name: Mao Song
  url: https://maosong.website/
published: 2025-12-05 10:09:06+0800
modified: 2025-12-05 10:09:06+0800
tags:
- Qwen
- RL
categories:
...
- Reasoning
showPdf: false
seoThumbImage: /posts/notes-on-sapo/SAPO-Qwen3-VL-perfornance.png
---
## Introduction

相关工作包括 GRPO 以及 [GSPO](/p/notes-on-gspo/)
...
![illustration of gating mechanism](/posts/notes-on-sapo/SAPO-gating-illustration.png)
...
![Performance of SAPO](/posts/notes-on-sapo/SAPO-performance.png)

实验结果显示，SAPO 的表现超过了 GSPO 以及 GRPO

作者还探究了超参数 $\tau_{pos}$ 和 $\tau_{neg}$ 的影响，结果如下图所示

![Ablation study on temperature](/posts/notes-on-sapo/SAPO-ablation-on-temperature.png)

实验结果显示，当 $\tau_{neg}>\tau_{pos}$ 时，模型训练最稳定，这说明了 negative token 是导致训练不稳定的主要原因。

作者还在 [Qwen3-VL](/p/notes-on-qwen3-vl/) 上进行了验证，结果如下图所示

![Performance of SAPO on Qwen3-VL](/posts/notes-on-sapo/SAPO-Qwen3-VL-perfornance.png)
