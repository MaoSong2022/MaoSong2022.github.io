...
作者还发现，模型表现提升是因为加入了 high-resolution image token 信息

不同的 projection layer 以及对应的模型如下表所示 [@xiao2026visionencodersurvey]

| Layer Type             | Models                                       |
| ---------------------- | -------------------------------------------- |
| Q-Former               | BLIP-2, InstructBLIP, MiniGPT-4, Intern-VL   |
| linear                 | LLaVA, PaliGemma 1/2, SmolVL                 |
| MLP                    | LLaVA-Next, InternVL 2.0+, Qwen2-VL          |
| Perceiver              | Flamingo, Idefics, Qwen-VL, LLaMA 3.2-Vision |
| visual embedding table | Ovis series                                  |
