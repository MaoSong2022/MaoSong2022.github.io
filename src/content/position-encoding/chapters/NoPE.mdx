...
[@kazemnejad2023the] 系统性对比了 [AliBi](/p/notes-on-alibi/), [RoPE](/p/notes-on-position-encoding/) , [T5](/p/notes-on-t5/) 提出的 T5 bias 以及 Transformer 提出的绝对位置编码 (APE).

作者发现，常用的方法在 length generalization 上表现并不是最好的，而 NoPE 不需要额外的计算开销反而效果最好。

[@wang-etal-2024-length] 进一步探究了 NoPE 长度外推的泛化性。作者有三点发现：
