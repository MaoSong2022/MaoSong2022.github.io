...
published: 2025-09-02 11:24:10+0800
modified: 2025-09-02 11:24:10+0800
tags:
- position embedding
- Attention
- Long Context
...
- MLLM
showPdf: false
seoThumbImage: /posts/notes-on-rnope-swa/RNoPE-SWA-128k-attention.png
---
import Image from '../../../components/Image.astro';
import rnope_swa_8k_attention from '../../../assets/image/RNoPE-SWA-8k-attention.png';
import rnope_swa_32k_attention from '../../../assets/image/RNoPE-SWA-32k-attention.png';
import rnope_swa_128k_attention from '../../../assets/image/RNoPE-SWA-128k-attention.png';
...
对于建模长上下文输入，我们可以从 attention 机制或者位置编码来入手。前者类似的工作有 Landmark Attention 和 Focused Transformer, 但是这些方法的问题在于训练不稳定。 [QK norm](/p/notes-on-qk-norm/) 可以比较好解决 softmax 分布过于极端的问题，但是其问题在于训练时的数值不稳定性，并且可能会影响模型的长上下文能力。

另一方面，对于位置编码，已有的工作如 APE, AliBi, [RoPE](/p/notes-on-position-encoding/) 等都可以提供位置信息。但是，这些方法很有可能回影响模型最终的 attention score 分布。另外，[NoPE](/p/notes-on-nope/) 探究了移除 position encoding 的可能性。

还有一些工作目的是降低 softmax attention 的时间复杂度和空间复杂度。比如 sliding window attention, sparse attention, [attention sink](/p/notes-on-streamingllm/) 等都可以降低整体的时间/空间复杂度。但是这些方法最终的表现都有所下降。
...
<Image src={rnope_swa_8k_attention} alt="Attention distribution on 8K context length" />

<Image src={rnope_swa_32k_attention} alt="Attention distribution on 32K context length" />

<Image src={rnope_swa_128k_attention} alt="Attention distribution on 128K context length" />
...
考虑到 [NoPE](/p/notes-on-nope/) 和 RopE 各自的优点，作者提出了一个混合架构，来结合 NoPE 与 RoPE. 具体做法就是 NoPE layer 和 RoPE layer 交替进行。作者将这个模型架构记为 RNoPE.
...
尽管本文和已有的工作如 YoCo, Jamba-1.5 和 [MiniMax-01](/p/notes-on-minimax-01/) 均证明了混合 attention 架构的有效性。但是，目前对于其工作机制尚缺乏一个比较系统性的理解。作者认为这是一个需要探究的方向。另外，作者还认为如何更好的汇总上下文信息与位置信息也是一个可以探究的方向。
