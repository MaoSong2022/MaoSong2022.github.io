...
前面的 policy gradient 和 actor-critic 给出了更新方向，但没有回答一个关键问题：
**一次可以把 policy 更新多远？** 如果新旧 policy 差异过大，旧 policy 生成的
trajectory 很快失效，近似目标也可能不再可靠。

TRPO (Trust Region Policy Optimization) [@TRPO] 使用旧策略
$\pi_{\theta_{\mathrm{old}}}$ 的 rollout 构造 surrogate objective，并通过 KL
constraint 限制新旧策略的距离。它仍然主要是 on-policy 方法；重点不是把
policy gradient 变成一般的 off-policy learning，而是在一个受控的 trust region
内安全地复用当前批次。

这一思想对 LLM 尤其重要：生成 rollout 成本很高，但在同一批 rollout 上执行过多
gradient steps 又会造成 policy drift。TRPO 首先形式化这一稳定性约束，后续 PPO
会给出更容易实现的一阶近似。
...
$$
\mathcal{J}(\theta) = \mathbb{E}_{\tau\sim(p_0,\textcolor{red}{\pi_\theta}, p),a_t'\sim\pi_{\theta_{old}}(\cdot\mid s_t)}\left[\sum_{t=0}^{T-1}\gamma^t\frac{\pi_\theta(a_t'\mid s_t)}{\pi_{\theta_{old}}(a_t'\mid s_t)}A^{\pi_{\theta_{old}}}(s_t,a_t')\right]+C
$$
...
<Algorithm title="TRPO MC">

1. Initialize policy parameters $\theta_0$, value function parameters $\phi_0$. Set hyperparameter $\delta > 0$.
2. **for** $k = 0, 1, \dots$:
    - Sample $N$ trajectories $\{\tau_i\}$ according to $(p_0, \pi_{\theta_k}, p)$.
    - **Solve the constrained optimization**:
      ...
      \end{aligned}
      $$
    - Compute $\hat{R}_t^{(i)} = \gamma^{t'-t} r_{t'}^{(i)}$ for $i = 1, \dots, N$, $t = 1, \dots, T^{(i)}-1$.
    - **Update the value model**:
      $$
      \min_{\phi} \frac{1}{N} \sum_{i=1}^{N} \frac{1}{T^{(i)}} \sum_{t=0}^{T^{(i)}-1} \frac{1}{2} \left(V_{\phi}(s_t^{(i)}) - \hat{R}_t^{(i)}\right)^2
      $$
