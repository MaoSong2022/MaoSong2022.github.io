...
Value-based methods 先学习 $Q^*$，再通过 $\arg\max_a Q^*(s,a)$ 间接得到策略。
对 LLM 而言，我们已经有一个参数化的 autoregressive policy $\pi_\theta(y_t\mid x,y_{<t})$, 因此更自然的选择是直接调整 $\theta$，提高高回报 response 的生成概率。
这就是 policy-based methods 的核心思想。

<Note title="Core idea" emoji="💡" variant="info">

使用 policy  $\pi_\theta$ 生成 trajectory, 通过 trajectory reward 构造 gradient estimator，再直接改进当前策略 $\pi_\theta$.
...
| :----------------: | :--------------------------------: | :---------------------: |
|      examples      |             PPO, GRPO              |        DQN, DDPG        |
|    sampled from    |           current policy           |    behavior policy     |
| data exploration   |    controlled by current policy    | controlled by behavior policy |
|  infra efficiency  | low (needs to generate new sample) |          high           |
| training stability | sensitive to policy update size     | sensitive to distribution shift |
|    compute cost    |                high                |           low           |
|      best for      | fresh rollout and direct policy optimization | reusing past or external data |

目前，主流针对 LLM 的 RL 算法基本都是

### Objective

我们的目标与前面一致，仍然是[最大化 return](#rl_objective), 问题形式为

$$
\max_{\pi}\mathcal{J}(\theta)=\mathbb{E}_{\tau\sim\pi_\theta}\left[G_0(\tau)\right]
$$
...
<Theorem title='Policy Gradient Theorem' id='policy_gradient_theorem'>

目标函数 [最大化 return](#rl_objective) 的梯度为
...
由 [trajectory definition](#trajectory_def), trajectory $\tau$ 的概率分布可以写为
...
$$
\begin{aligned}
\nabla_\theta \mathcal{J}(\theta)&=\nabla_\theta\mathbb{E}_{\tau\sim\pi_\theta}\left[G_0(\tau)\right]\\
