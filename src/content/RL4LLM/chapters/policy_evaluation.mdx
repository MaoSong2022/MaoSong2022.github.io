...
虽然我们上一节已经知道 $V^{\pi}$ 满足 Bellman equation，并且，我们可以使用一个迭代解法来求解，但当状态空间较大、动作空间较大时，直接进行精确求解往往计算代价较高.

在本节中，我们将要介绍如何高效求解 value function $V^{\pi}$.
...
这里我们使用 $V_{\phi}$ 来近似 $V^{\pi}$, 其中 $\phi$ 是函数的参数，使用最小二乘误差作为损失函数：

$$
\min_{\phi} \mathcal{L}(\phi) = \mathbb{E}_{s_0 \sim p_0}\left[\frac{1}{2}\left(V_{\phi}(s_0) - V^{\pi}(s_0)\right)^2\right]
$$
...
$$
\min_{\phi} \mathcal{L}(\phi) = \mathbb{E}_{s_0 \sim p_0}\left[\frac{1}{2}\left(V_{\phi}(s_0) - r_0 - \gamma V^{\pi}(s_1)\right)^2\right]
$$
