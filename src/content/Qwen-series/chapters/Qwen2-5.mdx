...
published: 2025-07-12 10:51:42+0800
modified: 2025-07-12 10:51:42+0800
tags:
- Qwen
categories:
- LLM
showPdf: false
seoThumbImage: /posts/notes-on-qwen2-5/Qwen2_5_32b_instruct_performance.png
---

import Image from '../../../components/Image.astro';
import qwen2_5_72b_instruct_performance from '../../../assets/image/Qwen2_5_72b_instruct_performance.png';
import qwen2_5_32b_instruct_performance from '../../../assets/image/Qwen2_5_32b_instruct_performance.png';
import qwen2_5_7b_instruct_performance from '../../../assets/image/Qwen2_5_7b_instruct_performance.png';
import qwen2_5_long_context_ttft from '../../../assets/image/Qwen2_5_long_context_TTFT.png';

2024 年 12 月 Qwen 发布了 Qwen 2.5 系列大语言模型，包括 7 个 dense 模型以及两个 MoE 模型，Qwen2.5 在 pre-training 阶段使用了 18T token 进行。在 post-training 阶段使用了 1M 的样本，还使用了 [DPO](/p/notes-on-dpo/) 以及 GRPO 来进行 RL 的训练

Qwen2.5 主要在以下方面进行了改进

1. 模型方面，提供了更多的 size，[Qwen2](/p/notes-on-qwen2/) 中只有 0.5B, 1.5B, 7B, 72B 四个 size, 在 Qwen2.5 中，加入了 3B, 14B 和 32B 三个 size 的模型
...
在提升模型上下文过程中，作者使用 ABF 技巧将 [Position Encoding](/p/notes-on-position-encoding/) 的 base frequency 从 10,000 提升到了 1,000,000.
...
为了提高模型在 inference 时的长上下文表现，作者使用了 [Dual Chunk Attention](/p/dual-chunk-attention/) 和 [YARN](/p/notes-on-yarn/) 两个技巧。通过这两个技巧，作者将 Qwen2.5-Turbo 的上锈阿文扩展到了 1M，将其他模型的上下文长度扩展到了 131072.
...
<Image src={qwen2_5_72b_instruct_performance} alt="Performance of Qwen2.5 72B" />

<Image src={qwen2_5_32b_instruct_performance} alt="Performance of Qwen2.5 32B" />

<Image src={qwen2_5_7b_instruct_performance} alt="Performance of Qwen2.5 7B" />

可以看到，Qwen2.5 72B 模型表现和 LLaMA3.1 405B 表现差不多，其他两个 size 的模型基本上达到了 SOTA

最后，作者评估了一下 DCA+YaRN v.s. Full attention 的表现，结果如下图所示

<Image src={qwen2_5_long_context_ttft} alt="TTFT of Qwen2.5 on long context" />
