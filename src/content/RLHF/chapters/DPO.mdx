...
为了解决这个问题，[@rafailov2023direct] 提出了 Direct Preference Optimization (DPO), DPO 通过构建 reward function 和最优策略之间的关系，进而通过训练 policy model 来同时完成 reward model 的训练。这样，我们就避免了 reward model 的训练。结果发现，DPO 的表现超过了之前的偏好优化方法。
...
def get_batch_logps(logits, labels, average_log_prob=False):
    ...
    per_token_lops = -loss * mask

    if average_log_prob:
        return per_token_lops.sum(dum=-1) / mask.sum(dim=-1)
    return per_token_lops.sum(dum=-1)
