---
title: KL divergence from machine learning to reinforcement learning 
...
published: 2026-01-24 16:32:14+0800
modified: 2026-01-24 16:32:14+0800
categories:
- Machine Learning
- math
- RL
showPdf: false
seoThumbImage: /posts/notes-on-kl-divergence/KL-divergence-estimator-gradient-bias.png
---
...
![visualization of forward KL v.s. reverse KL (1)](../../assets/image/KL-divergence-reverse-kl-vis1.png)

接下来我们改变 reverse KL 的初始化条件为 $\theta_0=(-2,1)$, 对应的结果为

![visualization of forward KL v.s. reverse KL (2)](../../assets/image/KL-divergence-reverse-kl-vis2.png)
...
$$
\frac{\pi_{ref}(x)}{\pi_\theta(x)} = 1+\epsilon(x), \text{ where } |\epsilon(x)|  \ll  1
$$
...
即当 $\pi_\theta$ 和 $\pi_{ref}$ 比较接近时，$\rho k_3$ 的 variance 比 $\rho k_1$ 更小，这是由于 $\rho s_\theta (k_1+1)$ 额外包含了一个 期望为零的项，这导致了其 variance 比较高。在 [DeepSeek-V3.2](/p/notes-on-deepseek-v3.2/) 中，作者就使用了 $\rho k_3$ 来降低梯度的 variance, 提高训练的稳定性。
...
![bias of KL divergence estimators and their gradients](../../assets/image/KL-divergence-estimator-gradient-bias.png)
