...
蚂蚁提出了针对 MoE 模型的 scaling law [@Ling-mini-beta], 并基于 scaling law 提出了 Ling-mini-beta

目前已经有了针对 dense LLM 的 scaling law, 如 [Kaplan scaling law](Kaplan%20scaling%20law.md) 和 [Chinchilla scaling law](Chinchilla%20scaling%20law.md).
...
与 [DeepSeek-LLM](DeepSeek-LLM.md) 一样，作者使用 $C=MD$ 来表示算力，non-embedding FLOPs $M$ 和训练 token 数 $D$ 之间的关系。
...
1. 与 [DeepSeek-V3](DeepSeek-V3.md) 一样，将 early layer 替换为 dense layer 可以避免 routing imbalance, 并且不会损害模型的表现
...
基于 Section 4 的发现，作者构建了 Ling-mini-beta, 一个 17.5B 总参数，激活参数为 0.85B 的 MoE 模型。训练使用了 1T token, 模型参数如下表所示
