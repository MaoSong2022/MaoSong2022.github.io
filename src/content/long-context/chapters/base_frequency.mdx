...
description: 百川探究了 LLM 中 RoPE base frequency 与 context length 之间的关系，给出了 base frequency
  与 context length 之间的 lower bound.
authors:
- name: Mao Song
  url: https://maosong.website/
published: 2025-12-22 11:34:42+0800
modified: 2025-12-22 11:34:42+0800
tags:
- RoPE
- position encoding
- Long context
...
- LLM
showPdf: false
seoThumbImage: /posts/base-of-rope-bounds-context-length/base-frequency-comparison-interpolation.png
---
import Image from '../../../components/Image.astro';
import base_frequency_fine_tuning_impact from '../../../assets/image/base-frequency-fine-tuning-impact.png';
import base_frequency_pre_training_stage from '../../../assets/image/base-frequency-pre-training-stage.png';
import base_frequency_comparison_interpolation from '../../../assets/image/base-frequency-comparison-interpolation.png';

## Introduction

[RoPE](/p/notes-on-position-encoding/) 已经成为了大多数 LLM 使用的 position encoding 范式，但是，RoPE 与 LLM long context 之间的关系还没有被探索清楚。在本文中，作者就探究了 base frequency 与 LLM context capability 之间的关系，并给出了一个达到指定上下文长度所需要的 base frequency 的 lower bound.

## Method

首先作者回顾了 attention 与 [RoPE](/p/notes-on-position-encoding/) 的定义， 关键就是 RoPE 这部分，如下所示
...
接下来，作者回顾了 [YARN](/p/notes-on-yarn/). 其核心思想在于，预训练阶段所有可能的 $\cos(t-s)\theta_i$ 都见过，才能保证模型的 OOD 表现
...
在 [RoPE](/p/notes-on-position-encoding/) 中，作者已经给出了 $A_{ij}$ 与相对距离 $|i-j|$ 之间的关系。因此第一个性质已经满足了。
...
<Image src={base_frequency_fine_tuning_impact} alt="impact of base frequency on fine-tuning stage" />
...
<Image src={base_frequency_pre_training_stage} alt="impact of base frequency on pre-training stage" />
...
<Image src={base_frequency_comparison_interpolation} alt="comparison with interpolation" />
