...
One-to-one的情况很简单，一个 process 与另一个 process 进行通信，通信通过 `send` 和 `recv` 完成。
...
<Accordion title="An example of send and recv">
...
`send` 和 `recv` 还有对应的 immediate 版本，即 `isend` 和 `irecv`，

`send/recv`的特点是在完成通信之前，两个 process 是锁住的。
与之相反，`isend/irecv` 则不会加锁，代码会继续执行然后返回 `Work` 对象，为了让通信顺利进行，我们可以在返回之前加入 `wait()`

<Accordion title="An example of isend and irecv">
...
Scatter 的作用是将一个 process 的数据均分并散布到其他 process,
Broadcast 的作用是将一个 process 的数据广播到其他 process.
两者不同的地方在于其他 process 获取到的是全量数据 (copy) 还是部分数据 (slice)，其示意图如下所示
...
$$
\mathrm{Scatter}([x_1,x_2,\dots,x_N]) = [x_1], [x_2],\dots, [x_N]
$$

Scatter 测试代码：

<Accordion title="An example of scatter">
...
$$
\mathrm{Broadcast}([x_1,\dots,x_N]) =[x_1,\dots,x_N], \dots, [x_1,\dots,x_N]
$$

Broadcast 测试代码:

<Accordion title="An example of broadcast">
...
Many-to-one 情形下，也可以分为两种：gather 和 reduce.

#### Gather

Gather 对应 one-to-many 的 scatter 操作，负责将多个 process 的内容汇聚到一起，形成一个完整的向量，对应的表达式如下

$$
\mathrm{Gather}([x_1],[x_2],\dots,[x_N]) = [x_1,x_2,\dots,x_N]
$$
...
<Accordion title="An example of gather">
...
Reduce 则是通过一个函数 $f(\cdot)$ 来把数据进行汇总，常见的函数有求和以及求平均，对应的表达式如下

$$
\mathrm{Reduce}([x_1],[x_2],\dots,[x_N], f) = [f(x_1,x_2,x_N)]
$$
...
<Accordion title="An example of reduce">
...
Many-to-many 情形下的两种通信方式为：All-Reduce 和 All-Gather，分别是reduce和gather的升级版。

#### AllReduce

AllGather 则对所有process执行一次gather操作，其示意图如下所示

<Image src={all_gather_reduce} alt="all-gather and all-reduce" />

AllGather 测试代码：

<Accordion title="An example of all-gather">
...
$$
\htmlId{all-reduce}{\begin{equation}
\mathrm{AllReduce}([x_1,\dots,x_N]) \to \left[\frac{1}{N}\sum_{i=1}^Nx_i,\dots, \frac{1}{N}\sum_{i=1}^Nx_i\right]
\end{equation}
}
$$
...
AllReduce 对所有 process 都执行一次 reduce 操作

all-reduce 测试代码：

<Accordion title="An example of all-reduce">
...
<Accordion title="An example of barrier">
