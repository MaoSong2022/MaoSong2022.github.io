...
published: 2025-12-11 16:10:08+0800
modified: 2025-12-11 16:10:08+0800
tags:
- MoE
categories:
- LLM
- Tutorial
showPdf: false
seoThumbImage: /posts/load-balancing-tutorial/ollmoe-load-balancing-expert-assignment.png
---
...
[GShard](/p/gshard/) 进一步对 (Noam et.al 2017) 提出的 loss 进行了简化，首先注意到 $\sum_{i=1}^N\mathrm{Load}_i(X)=BK$, 因此我们有
...
[Switch Transformer](/p/switch-transformer/) 进一步对 [GShard](/p/gshard/) 提出 load balancing loss 进行了规范化，也是现在大部分 load balancing loss 使用的形式，其定义如下：
...
目前关于 load balancing loss 可以实现负载均衡的数学分析是比较少的。[Switch Transformer](/p/switch-transformer/) 中进行了如下分析：
...
<Image src={olmoe_load_balancing_loss} alt="Ablation study on load balancing loss" />

可以看到，加入 load balancing loss 之后，模型的表现均超过了不加时的表现。

作者进一步分析了不同专家在加/不加 load balancing loss 时的激活情况，结果如下图所示

<Image src={ollmoe_load_balancing_expert_assignment} alt="visualization of expert assignment on load balancing" />
