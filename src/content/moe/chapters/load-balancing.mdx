...
我们在 [MoE tutorial](/p/moe-tutorial/) 中已经介绍了 MoE 模块，MoE 尽管可以在相同的算力下扩大模型的 size, 但是其问题在于训练时容易出现负载不均衡，也就是说只有少数几个专家被激活，其他专家处于闲置状态，从而导致模型性能下降
...
```mermaid
flowchart TD;
    C[Bengio et.at 2015] --> A[Shazeer];
    A --> B[GShard];
    B --> F[SwitchTransformer];
...
[@shazeer2017] 进行了改进，提出了变异系数 $\mathrm{CV}(\cdot)$ 来保证负载均衡，其定义如下：
...
Switch Transformer [@SwitchTransformer] 进一步对 GShard [@GShard] 提出 load balancing loss 进行了规范化，也是现在大部分 load balancing loss 使用的形式，其定义如下：
...
我找了很多资料，最后发现苏剑林老师在 [@kexuefm-10735] 这篇 blog 中进行了分析。我这里基于苏剑林老师的 blog 进行了一下总结，推荐大家看苏剑林老师的原文。
...
具体细节见 [Loss-Free Balancing](/p/notes-on-loss-free-balancing/).
...
具体细节见 [Global-batch load balancing](/p/notes-on-global-batch-load-balancing/)

### Experiments

[olmoe](/p/notes-on-olmoe/) 对比了加入 load balancing loss 之后模型的表现变化情况，结果如下图所示
