...
作者还发现，later layers 比 early layers 饱和更快，early layer, 特别是 layer 0, 饱和的非常慢。作者认为，这是 DeepSeekMoE [@DeepSeek-MoE] 放弃在第一层使用 MoE layer 的原因，因为 load balancing loss 收敛更慢。后续 DeepSeek-V2 [@DeepSeek-V2] 和 DeepSeek-V3 [@DeepSeek-V3] 均在 early layer 上使用 dense layer 替换掉了 MoE layer
