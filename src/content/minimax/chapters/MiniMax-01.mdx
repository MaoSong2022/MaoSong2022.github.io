...
description: MiniMax-01 是一个基于 hybrid attention 架构的大模型系列，包含 MiniMax-Text-01 和 MiniMax-VL-01
  两个模型，其中 MiniMax-Text-01 推理时支持 4M 的上下文长度，MiniMax-VL-01 支持 512B 的上下文长度
authors:
- name: Mao Song
...
import minimax_01_dense_vs_moe from '../../../assets/image/MiniMax-01-dense-vs-MoE.png';
import minimax_01_scaling_law from '../../../assets/image/MiniMax-01-scaling-law.png';
import minimax_01_performance_attention from '../../../assets/image/MiniMax-01-performance-attention.png';
import minimax_01_attention_speed_comparison from '../../../assets/image/Minimax-01-attention-speed-comparison.png';
import minimax_01_hybrid_attention_comparison from '../../../assets/image/Minimax-01-hybrid-attention-comparison.png';
import minimax_01_hybrid_window_comparison from '../../../assets/image/MiniMax-01-hybrid-window-comparison.png';
import minimax_01_module_ablation from '../../../assets/image/MiniMax-01-module-ablation.png';
import minimax_01_ep_overlap from '../../../assets/image/MiniMax-01-EP-overlap.png';
import minimax_01_ep_etp_overlap from '../../../assets/image/MiniMax-01-EP-ETP-overlap.png';
import minimax_01_varlen_ring_attention from '../../../assets/image/MiniMax-01-varlen-ring-attention.png';
import minimax_01_text_performance from '../../../assets/image/MiniMax-01-text-performance.png';
import minimax_01_text_ruler_performance from '../../../assets/image/MiniMax-01-text-Ruler-performance.png';
import minimax_01_vl_data_distribution from '../../../assets/image/MiniMax-01-VL-data-distribution.png';
import minimax_01_vl_performance from '../../../assets/image/MiniMax-01-VL-performance.png';
...
![minimax_text_01_architecture](/posts/notes-on-minimax-01/minimax_text_01_architecture.png)
...
3. softmax attention 使用了 [GQA](/p/notes-on-gqa/), 来提高内存加载效率, group size 为 8
...
<Image src={minimax_01_dense_vs_moe} alt="Performance of MoE v.s. Dense" />
...
**Global router**
作者还基于 [GShard](/p/gshard/) 构建了一个 global routing 策略，由于 GPU memory 限制，对于每一个 micro batch size, token 分布不仅在一个 EP group 内分布不平衡，在不同 EP group 之间可能也不平衡。因此，作者实现了一个 global token dispatching 策略。具体来说，在 token 分发到不同的 EP group 之前，作者使用 `allgather` 来计算每个专家需要处理的 token. 这样就可以基于全局信息智能分配 token, 避免某些专家过载。
...
**Scaling law**
作者接下来分别针对三个架构设计了 70M, 160M, 410M, 1B, 3B, 7B 系列模型，模型使用 300B token 进行训练，上下文长度为 8192, 对于每种架构，作者将 batch size 设置为 4M tokens. 与 [Chinchilla scaling law](/p/chinchilla-scaling-law/) 类似， 作者探究了以下模型的 scaling law, 结果如下图所示

<Image src={minimax_01_scaling_law} alt="Summary of Scaling law" />
...
<Image src={minimax_01_performance_attention} alt="Performance of different attention architectures" />
...
<Image src={minimax_01_attention_speed_comparison} alt="Training speed of various attention mechanisms" />
...
<Image src={minimax_01_hybrid_attention_comparison} alt="Performance of various hybrid-linear models" />
...
<Image src={minimax_01_hybrid_window_comparison} alt="Comparison between hybrid lighting and hybrid window" />
...
<Image src={minimax_01_module_ablation} alt="Module ablations" />
...
<Image src={minimax_01_ep_overlap} alt="Expert parallel overlap illustration" />
...
<Image src={minimax_01_ep_etp_overlap} alt="EP-ETP overlap" />
...
当前主要是使用 ring attention 来划分数据，但是 ring-attention 与 sequence packing 是冲突的。已有工作如 [flash attention](/p/chinchilla-scaling-law/) 支持 varlen data packing, 但是不支持 ring attention; 而 TransformerEngine 实现了 ring attention, 但不支持真正的 varlen. 因此作者的目的就是解决这个问题。
...
<Image src={minimax_01_varlen_ring_attention} alt="Varlen Ring Attention" />
...
<Image src={minimax_01_text_performance} alt="Performance of MiniMax-Text-01" />

作者还评估了以下 MiniMax-Text-01 在长上下文情境下表现，我们这里列出 Ruler 上的结果，如下图所示

<Image src={minimax_01_text_ruler_performance} alt="MiniMax-Text-01 performance on Ruler benchmark" />
...
<Image src={minimax_01_vl_data_distribution} alt="Visualization of top rags of sampled instruction data" />
...
这个阶段的目的是对齐，作者使用 [DPO](/p/notes-on-dpo/) 来训练模型，数据集包括 **40,000** 条数据。数据构建过程如下：
...
<Image src={minimax_01_vl_performance} alt="Performance of MiniMax-VL-01" />
