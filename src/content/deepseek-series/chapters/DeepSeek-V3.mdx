...
description: DeepSeek 在 24 年 11 月发布了 DeepSeek-V3, 一个仅花费 2.8M H800 hours 的大语言模型，且在各个
  benchmark 上达到了 SOTA 表现
authors:
- name: Mao Song
  url: https://maosong.website/
published: 2025-12-08 11:14:45+0800
modified: 2025-12-08 11:14:45+0800
tags:
- deepseek
- MoE
- Reasoning
...
- LLM
showPdf: false
seoThumbImage: /posts/notes-on-deepseek-v3/DeepSeek-V2-architecture.png
---
import Image from '../../../components/Image.astro';
import deepseek_v2_architecture from '../../../assets/image/DeepSeek-V2-architecture.png';
import deepseek_v3_mtp from '../../../assets/image/DeepSeek-V3-MTP.png';
import deepseek_v3_overlapping_strategy from '../../../assets/image/DeepSeek-V3-overlapping-strategy.png';
import deepseek_v3_dualpipe_scheduling from '../../../assets/image/DeepSeek-V3-DualPipe-scheduling.png';
import gpu_communication_pattern from '../../../assets/image/GPU-communication-pattern.png';
import deepseek_v3_mixed_precision from '../../../assets/image/DeepSeek-V3-mixed-precision.png';
import deepseek_v3_quantization from '../../../assets/image/DeepSeek-V3-quantization.png';
import deepseek_v3_fp8_performance from '../../../assets/image/DeepSeek-V3-FP8-performance.png';
import deepseek_v3_prefilling_overlap from '../../../assets/image/DeepSeek-V3-prefilling-overlap.png';
import deepseek_v3_decoding_overlap from '../../../assets/image/DeepSeek-V3-decoding-overlap.png';
import deepseek_v3_base_performance from '../../../assets/image/DeepSeek-V3-base-performance.png';
import deepseek_v3_ablation_mtp from '../../../assets/image/DeepSeek-V3-ablation-MTP.png';
import deepseek_v3_ablation_loss_free_balancing from '../../../assets/image/DeepSeek-V3-ablation-loss-free-balancing.png';
import deepseek_v3_ablation_batch_wise from '../../../assets/image/DeepSeek-V3-ablation-batch-wise.png';
import deepseek_v3_instruct_performance from '../../../assets/image/DeepSeek-V3-instruct-performance.png';
...
1. efficiency inference: 采用了 [DeepSeek-V2](/p/notes-on-deepseek-v2/) 提出的 [MLA](/p/notes-on-mla/)
2. cost-effective training: 采用了 [DeepSeekMoE](/p/notes-on-deepseekmoe/) 提出了 MoE 架构
3. auxiliary-loss-free strategy: 采用了 [Loss-Free Balancing](/p/notes-on-loss-free-balancing/) 提出的 loss balancing 策略
...
后训练阶段，作者使用了 SFT 和 RL 两个阶段来提高模型的表现，作者还对 [DeepSeek-R1](/p/notes-on-deepseek-r1/) 进行蒸馏来提高模型的 reasoning 能力

## Architecture

### Basic Architecture

DeepSeek-V3 的架构与 [DeepSeek-V2](/p/notes-on-deepseek-v2/) 的架构一致，如下图所示

<Image src={deepseek_v2_architecture} alt="Architecture of DeepSeek-V2" />

MLA 的介绍见 [MLA](/p/notes-on-mla/), MoE 架构的介绍见 [DeepSeekMoE](/p/notes-on-deepseekmoe/). DeepSeek-V3 在 DeepSeekMoE 的基础上做了两点改变：

1. 受 [Loss-Free Balancing](/p/notes-on-loss-free-balancing/) 启发，作者使用了 sigmoid fu nction 来计算 affinity score
2. 对于 selected affinity score 应用了 normalization

在 [DeepSeekMoE](/p/notes-on-deepseekmoe/) 的基础上，作者使用了 [Loss-Free Balancing](/p/notes-on-loss-free-balancing/). 其表达式如下
...
与 [DeepSeekMoE](/p/notes-on-deepseekmoe/) 一样，作者也在 node 层面实现负载均衡，具体做法就是，根据 node 中所包含 expert 的 affinity score 之和来选取最高的 $M$ 个 nodes, 这样就可以进一步提高 computation 和 communication 之间的 overlap.
...
<Image src={deepseek_v3_mtp} alt="Illustration of MTP" />
...
与之前的 DeepSeek 系列相同，DeepSeek-V3 也是用了 HAI-LLM 框架来支持训练。训练时，DeepSeek-V3 使用了 16-way PP, 64-way EP (spanning 8 nodes, [GShard](/p/gshard/)) 以及 ZeRO-1 DP.
...
<Image src={deepseek_v3_overlapping_strategy} alt="Overlapping stategy of DeepSeek-V3" />
...
<Image src={deepseek_v3_dualpipe_scheduling} alt="DualPipe scheduling" />
...
<Image src={gpu_communication_pattern} alt="Communication of GPU" />

对于 H800 来说,NVLink 的带宽为 160GB/s, IB 的带宽为 50GB/s, 因此 NVLInk 的通信效率是 IB 的 3.2 倍，即节点内部通信效率高于节点之间的通信效率。为了提高通信效率，作者限制每个 token 只能被分发到至多 4 个节点上（[DeepSeek-V2](/p/notes-on-deepseek-v2/) 里提到的 device limited routing）。其具体通信方式为想传输到目标 node index 相同的 rank 上，然后再通过节点内部通信传输到目标的 rank 上。通过这种方式，我们可以让节点间通信与节点内部通信进行重叠，从而每个 token 可以从一个节点上选取 3.2 个专家，这样 DeepSeek-V3 可以在不损失效率的情况下最高选取 $13$ 个专家。
...
<Image src={deepseek_v3_mixed_precision} alt="Mix-precision training of DeepSeek-V3" />
...
<Image src={deepseek_v3_quantization} alt="Quantization of DeepSeek-V3" />
...
- Low-precision optimizer states: 对于 optimizer states, 作者使用了 BF16 数据格式来保存 [AdamW](/p/notes-on-adamw/) 的优化器的一阶和二阶动量，但是对于 master weight 和 gradients 作者仍然使用了 FP32 来保证训练的数值稳定性
...
<Image src={deepseek_v3_fp8_performance} alt="FP8 v.s. BF16" />
...
<Image src={deepseek_v3_prefilling_overlap} alt="Prefiling overlap of DeepSeek-V3" />
...
<Image src={deepseek_v3_decoding_overlap} alt="Decoding overlap of DeepSeek-V3" />
...
相比于 [DeepSeek-V2](/p/notes-on-deepseek-v2/), DeepSeek-V3 提升了数学和代码数据的比例，以及增加了多语种数据。最终训练数据一共包括 **14.8T**
...
作者使用了 [YARN](/p/notes-on-yarn/) 来扩展模型的上下文长度，模型上下文长度经过两个额外的训练阶段从 4K 扩展到 32K 再扩展到 128K, 均训练了 1000 步。

YARN 配置与 [DeepSeek-V2](/p/notes-on-deepseek-v2/) 基本一致，在额外的 decoupled query and key 上作者没有应用这一点。参数配置如下表
...
DeepSeek-V3 base 的表现下图所示，作者对比了 [DeepSeek-V2](/p/notes-on-deepseek-v2/), [Qwen2.5](/p/notes-on-qwen2.5/), [LLaMA 3.1](LLaMA%203.1)

<Image src={deepseek_v3_base_performance} alt="Performance of DeepSeek-V3 base" />

### Discussion

作者首先验证了 MTP 的有效性，结果如下图所示

<Image src={deepseek_v3_ablation_mtp} alt="Ablation study on MTP" />

可以看到，在模型架构相同，训练数据相同的情况下，1-MTP 的效果超过了 baseline 的表现，说明了 MTP 策略的有效性

接下来，作者还验证了 [Loss-Free Balancing](/p/notes-on-loss-free-balancing/) 的有效性，结果如下图所示

<Image src={deepseek_v3_ablation_loss_free_balancing} alt="Ablation on loss-free balancing" />
...
<Image src={deepseek_v3_ablation_batch_wise} alt="Ablation study on batch-wise load balancing" />
...
post-training 包含 1.5M 样本，数据包括 reasoning 数据以及 non-reasoning 数据，前者由 [DeepSeek-R1](/p/notes-on-deepseek-r1/) 合成，后者由 DeepSeek-V2.5 合成
...
<Image src={deepseek_v3_instruct_performance} alt="Performance of DeepSeek-V3" />

### Post-training Discussion

作者首先探究了 Distillation 对模型表现的影响，作者使用 [DeepSeek-R1](/p/notes-on-deepseek-r1/) 来蒸馏 DeepSeek-V2.5, 结果如下图所示
