...
- 预训练阶段，DeepSeek-V3 使用了**14.8T token**.
...
DeepSeek-V3 的架构与 [DeepSeek-V2](#deepSeek-v2) 的架构一致，见 [DeepSeek-V2](#deepseek-v2).

DeepSeek-V3 在 DeepSeekMoE 的基础上，使用 sigmoid function 来单独计算每个专家的贡献，然后再进行 normalization.

DeepSeek-V3 还使用了 [Loss-Free Balancing](/p/notes-on-loss-free-balancing/), 其表达式如下
...
其中 $b_i$ 是一个随机扰动，仅影响 routing 结果。
训练时，如果对应的 expert 负载不均衡，则对 $b_i$ 进行更新，更新方式为 $b_i\pm\gamma$, 这里 $\gamma$ 是一个超参数。
...
受 MTP [@MTP] 启发，DeepSeek-V3 也构建了 MTP 模块用于预测未来的多个 token, 作者认为 MTP 模块有两个优势：

1. MTP objective 提供了更多的学习信号，进而提高了数据使用效率
2. MTP 可以让模型更好预测未来的 token, 提高模型的规划能力

DeepSeek-V3 所使用的 MTP 模块架构图如下所示

<Image src={deepseek_v3_mtp} alt="Illustration of MTP" />

MTP 模块使用了 $D$ 个 sequential modules 来预测未来的 $D$ 个 token.
其中，第 $k$ 个 MTP 模块包含：

- 一个共享的 embedding layer $\mathrm{Emb}(\cdot)$
- 一个共享的 output head $\mathrm{OutHead}(\cdot)$
- 一个 transformer block $\mathrm{TRM}_k(\cdot)$
- 一个 projection matrix $M_k\in\mathbb{R}^{d\times 2d}$.

对于第 $i$ 个 token 以及第 $k$ 个 MTP 模块，作者首先将第 $k-1$ 个 MTP 模块的第 $i$ 个 token 的 hidden states $h_i^{k-1}\in\mathbb{R}^d$ 和第 $i+k$ 个 token 的 embedding $\mathrm{Emb}(t_{i+1})\in\mathbb{R}^d$ concate 在一起

$$
h_i'^{k}=M_k[\mathrm{RMSNorm}(h_{i}^{k-1});\mathrm{RMSNorm}(\mathrm{Emb}(t_{i+k}))]
$$

$h_{i}^{0}=h_i$ 是 main model 的输出。
每个 MTP 模块的 embedding layer 和 main model 的 embedding layer 是共享的，接下来， $h_i'^k$ 作为第 $k$ 个 MTP 模块 transformer block 的输入，得到
...
<Image
  src={deepseek_v3_overlapping_strategy}
...
<Image
  src={deepseek_v3_mixed_precision}
...
<Image
  src={deepseek_v3_prefilling_overlap}
...
<Image
  src={deepseek_v3_decoding_overlap}
...
DeepSeek-V3 base 的表现下图所示，作者对比了 DeepSeek-V2 [@DeepSeek-V2], [Qwen2.5](/p/notes-on-qwen2.5/), LLaMA 3.1 [@LLaMA3]

<Image
  src={deepseek_v3_base_performance}
...
<Image
  src={deepseek_v3_ablation_loss_free_balancing}
...
<Image
  src={deepseek_v3_ablation_batch_wise}
...
<Image
  src={deepseek_v3_instruct_performance}
