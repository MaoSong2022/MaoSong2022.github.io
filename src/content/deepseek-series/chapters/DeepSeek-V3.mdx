...
MLA 的介绍见 [MLA](/p/notes-on-mla/), MoE 架构的介绍见 DeepSeekMoE [@DeepSeek-MoE]. DeepSeek-V3 在 DeepSeekMoE 的基础上做了两点改变：
...
在 DeepSeekMoE [@DeepSeek-MoE] 的基础上，作者使用了 [Loss-Free Balancing](/p/notes-on-loss-free-balancing/). 其表达式如下
...
与 DeepSeekMoE [@DeepSeek-MoE] 一样，作者也在 node 层面实现负载均衡，具体做法就是，根据 node 中所包含 expert 的 affinity score 之和来选取最高的 $M$ 个 nodes, 这样就可以进一步提高 computation 和 communication 之间的 overlap.
