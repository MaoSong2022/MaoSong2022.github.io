...
description: DeepSeek 在 2024 年 5 月提出了 DeepSeek-V2，一个基于 MoE 架构的大语言模型，参数量为 236B-A21B.
  作者使用了 MLA 来压缩 KV cache, 使用 DeepSeekMoE 架构来提高模型训练效率和表现。
authors:
- name: Mao Song
  url: https://maosong.website/
published: 2025-12-02 18:21:54+0800
modified: 2025-12-02 18:21:54+0800
tags:
- deepseek
- MoE
categories:
- LLM
showPdf: false
seoThumbImage: /posts/notes-on-deepseek-v2/DeepSeek-V2-architecture.png
---
import Image from '../../../components/Image.astro';
import deepseek_v2_architecture from '../../../assets/image/DeepSeek-V2-architecture.png';
import deepseek_v2_pretraining_performance from '../../../assets/image/DeepSeek-V2-pretraining-performance.png';
import deepseek_v2_posttraining_performance from '../../../assets/image/DeepSeek-V2-posttraining-performance.png';
...
1. 基于 [DeepSeekMoE](/p/notes-on-deepseekmoe/), 使用了 MoE 架构
2. 使用了 MLA 压缩 KV cache, 大幅度提高推理效率

DeepSeek-V2 预训练使用了 **8.1T** tokens, 相比于 [DeepSeek-LLM](/p/notes-on-deepseek-llm/), 预训练数据主要增加了中文数据以及提高了数据的质量。
...
关于架构的介绍见 [DeepSeekMoE](/p/notes-on-deepseekmoe/)
...
作者使用了三个 loss 来实现负载均衡。其中，expert level 和 device level 的 load balancing loss 与 [DeepSeekMoE](/p/notes-on-deepseekmoe/) 相同。第三个 loss 是 communication balance loss, 这个 loss 的目的是让每个设备的通信开销保持平衡。损失函数的表达式如下所示
...
具体做法就是，在训练之前，先将每个设备的 capacity factor 设置为 1 （定义见 [Switch Transformer](/p/switch-transformer/)）. 然后按照 affinity score 来丢弃一些分数比较低的 token, 直到该设备的 token 数量刚到达到 capacity。为了避免过度学习导致模型表现较差，对于 $10\%$ 的训练数据，作者不执行 token dropping 策略。
...
预训练数据与 [DeepSeek-LLM](/p/notes-on-deepseek-llm/) 基本上差不多，作者针对中文数据，数据质量进行了改进。最终预训练数据包括 **8.1T** token, 其中中文数据比英文数据多 $12\%$.

tokenizer 与 [DeepSeek-LLM](/p/notes-on-deepseek-llm/) 一致。
...
这里比较特殊的一点在于，模型在第一层使用了 MoE layer, 这个做法的原因在后面的 [olmoe](/p/notes-on-olmoe/) 里有提到，核心思想是 early layer 特别是第一层 layer 收敛比较慢。
...
训练的配置也与 [DeepSeek-LLM](/p/notes-on-deepseek-llm/) 差不多，对于 MoE，作者使用了 PP 将不同的 layers 分配在不同的 device 上，然后 MoE 的 experts 被分配在 8 个 device 上 ($D=8$), 对于 device-limited routing, 每个 token 发送到至多 3 个 device, 也就是 $M=3$.
...
在预训练阶段结束之后，作者使用了 [YARN](/p/notes-on-yarn/) 来将模型的上下文从 4K 扩展到 128K. 超参数设置为
...
作者对比了 [DeepSeek-LLM](/p/notes-on-deepseek-llm/), [Qwen1.5](/p/notes-on-qwen1.5/), [Mixtral MoE](/p/mixstral-8x7b/) 以及 [LLaMA 3](/p/notes-on-llama3/), 实验结果如下

<Image src={deepseek_v2_pretraining_performance} alt="Performance of DeepSeek-V2 base" />
...
<Image src={deepseek_v2_posttraining_performance} alt="Performance of DeepSeek-V2-chat" />
