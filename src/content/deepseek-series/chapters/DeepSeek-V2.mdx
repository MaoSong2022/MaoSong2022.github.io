...
DeepSeek-V2 预训练使用了 **8.1T** tokens, 相比于 DeepSeek-LLM [@DeepSeek-LLM], 预训练数据主要增加了中文数据以及提高了数据的质量。
...
预训练数据与 DeepSeek-LLM [@DeepSeek-LLM] 基本上差不多，作者针对中文数据，数据质量进行了改进。最终预训练数据包括 **8.1T** token, 其中中文数据比英文数据多 $12\%$.

tokenizer 与 DeepSeek-LLM [@DeepSeek-LLM] 一致。
...
训练的配置也与 DeepSeek-LLM [@DeepSeek-LLM] 差不多，对于 MoE，作者使用了 PP 将不同的 layers 分配在不同的 device 上，然后 MoE 的 experts 被分配在 8 个 device 上 ($D=8$), 对于 device-limited routing, 每个 token 发送到至多 3 个 device, 也就是 $M=3$.
...
作者对比了 DeepSeek-LLM [@DeepSeek-LLM], [Qwen1.5](/p/notes-on-qwen1.5/), [Mixtral MoE](/p/mixstral-8x7b/) 以及 LLaMA3 [@LLaMA3], 实验结果如下

<Image
  src={deepseek_v2_pretraining_performance}
...
<Image
  src={deepseek_v2_posttraining_performance}
