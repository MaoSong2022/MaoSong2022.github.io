...
description: DeepSeek 在 2024 年 1 月 5 日发布了 DeepSeek LLM, 包括 7B 和 67B 两个 size, 作者主要强调了对于
  scaling law 的探究
authors:
- name: Mao Song
...
import deepseek_llm_learning_rate_ablation from '../../../assets/image/DeepSeek-LLM-learning-rate-ablation.png';
import deepseek_llm_scaling_curve_bsz_lr from '../../../assets/image/DeepSeek-LLM-scaling-curve-bsz-lr.png';
import deepseek_llm_isoflop_curve from '../../../assets/image/DeepSeek-LLM-IsoFLOP-curve.png';
import deepseek_llm_performance_scaling_curve from '../../../assets/image/DeepSeek-LLM-performance-scaling-curve.png';
...
最终，基于这些实验结果，作者提出了 DeepSeek LLM, 模型使用 **2T token** 进行预训练，使用 1M samples 进行后训练，后训练包括 SFT 以及 [DPO](/p/notes-on-dpo/).

## Pre-training

### Architecture

DeepSeek-LLM 的架构与 LLaMA 基本相同，作者在 67B 的模型上使用了 [GQA](/p/notes-on-gqa/) 来提高 inference 效率。最终模型的配置如下表所示
...
对于 tokenizer, 作者使用了 [BBPE](/p/hands-on-llm1-tokenizer/) 算法，tokenizer 的大小设置为 100,000, 最终的 tokenizer 大小为 102400.
...
<Image src={deepseek_llm_learning_rate_ablation} alt="Comparison of different learning schedulers" />
...
作者使用了数据并行，张量并行，序列并行以及 1F1B pipeline 并行。作者还使用了 [flash attention](/p/notes-on-flashattention/) 来提高硬件利用率。
...
<Image src={deepseek_llm_scaling_curve_bsz_lr} alt="Scaling curves of batch size and learning rate" />
...
compute budget 与 model scale, data scale 之间的关系可以近似表示为 $C=6ND$, 这个公式的推导见 [LLM FLOPs computation](/p/llm-flops-computation/)。我们用 $N_1,N_2$ 分别表示模型的 non-embedding parameter 以及 complete parameters, 则我们可以用 $6N_1$ 或者 $6N_2$ 来近似 model scale, 但是 $6N_1$ 和 $6N_2$ 均没有考虑 attention 的计算开销，因此这两种近似的误差都比较大。
...
作者使用了 [Chinchilla](/p/chinchilla-scaling-law/) 提出来的 IsoFLOP 曲线进行拟合，实验结果如下图所示

<Image src={deepseek_llm_isoflop_curve} alt="IsoFLOP curve and optimal model/data allocation" />
...
<Image src={deepseek_llm_performance_scaling_curve} alt="Performance Scaling curve" />
...
<Image src={deepseek_llm_dataset_ablation} alt="Comparison of different dataset scaling" />
...
2. [DPO](/p/notes-on-dpo/): 提高模型的能力，作者发现 DPO 可以提高模型 open-ended generation skill.
