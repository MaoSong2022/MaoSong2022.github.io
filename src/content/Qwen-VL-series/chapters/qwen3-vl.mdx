...
published: 2025-12-05 10:12:01+0800
modified: 2025-12-05 10:12:01+0800
tags:
- Qwen
categories:
- MLLM
showPdf: false
seoThumbImage: /posts/notes-on-qwen3-vl/Qwen-VL-training-pipeline.png
---

import Image from '../../../components/Image.astro';
import qwen3_vl_architecture from '../../../assets/image/Qwen3-VL-architecture.png';
import qwen3_vl_deepstack from '../../../assets/image/Qwen3-VL-DeepStack.png';
import qwen3_vl_pretraining_recipe from '../../../assets/image/Qwen3-VL-pre-training-recipe.png';
import qwen3_vl_post_training_pipeline from '../../../assets/image/Qwen3-VL-post-training-pipeline.png';
import qwen3_vl_performance from '../../../assets/image/Qwen3-VL-performance.png';
import qwen3_vl_ablation_on_vit from '../../../assets/image/Qwen3-VL-ablation-on-ViT.png';
import qwen3_vl_ablation_on_deepstack from '../../../assets/image/Qwen3-VL-Ablation-on-Deepstack.png';
...
1. Interleaved MRoPE: 作者解决了 [Qwen2.5-VL](/p/notes-on-qwen2.5-vl/) 提出的 MRoPE 在长视频理解场景下的频谱不平衡问题
2. DeepStack: 作者使用了 DeepStack 来提取 [ViT](/p/notes-on-vit/) 不同 layer 的视觉特征
...
<Image src={qwen3_vl_architecture} alt="Architecture of Qwen3-VL" />

其中，

- LLM: LLM 使用了 [Qwen3](/p/notes-on-qwen3/) 系列大语言模型，包括 2B, 4B, 8B, 32B 四个 dense model 以及 30B-A3B, 235B-A22B 两个 moe 模型
- Vision Encoder: encoder 基于 [[SigLip-2]] 初始化，然后使用了 dynamic input resolutions 进行 continue training, 作者使用了 [CoMP](/p/notes-on-comp/) 提出的 2D-RoPE 以及 interpolate absolute position embedding, 最终包括 SigLip2-SO-400M 和 SigLip-Large (300M) 两个 size, 后者用于 2B 和 4B 两个 size
...
<Image src={qwen3_vl_deepstack} alt="Architecture of DeepStack" />

### Video Timestamp

作者发现，[Qwen2.5-VL](/p/notes-on-qwen2.5-vl/) 中使用的 MRoPE 存在如下问题：
...
<Image src={qwen3_vl_pretraining_recipe} alt="Qwen3-VL pretraining recipe" />
...
2. Strong-to-Weak Distillation: 提高小模型的能力，这里应该是和 [Qwen3](/p/notes-on-qwen3/) 一样，将大模型的能力蒸馏到小模型里
...
<Image src={qwen3_vl_post_training_pipeline} alt="Post-training pipeline of Qwen3-VL (guessed)" />
...
训练时，作者使用了 [SAPO](/p/notes-on-sapo/) 算法进行训练
...
<Image src={qwen3_vl_performance} alt="Performance of Qwen3-VL 235B-A22B" />

### Ablation Study

作者对比了以下 Qwen3-ViT 和 SigLIP-2 的表现，结果如下图所示

<Image src={qwen3_vl_ablation_on_vit} alt="Ablation on Qwen3-ViT" />
...
<Image src={qwen3_vl_ablation_on_deepstack} alt="Ablation on DeepStack" />

可以看到，相比于 baseline, DeepStack 的表现更好，说明了 DeepStack 可以提供更丰富的视觉信息。

作者还评估了以下 Qwen3-VL 在视频版大海捞针任务上的表现，实验结果发现，对于 30 分钟的视频，Qwen3-VL 的准确率为 $100\%$, 通过 [YARN](/p/notes-on-yarn/) 上下文扩展策略，模型在 2 个小时视频上的准确率为 $99.5\%$.
