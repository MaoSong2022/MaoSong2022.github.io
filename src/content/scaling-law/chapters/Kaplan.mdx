...
import Kaplan_model_shape_ablation from '../../../assets/image/Kaplan-model-shape-ablation.png';
import Kaplan_ablation_model_size from '../../../assets/image/Kaplan-ablation-model-size.png';
import Kaplan_parameter_scaling_law_curve from '../../../assets/image/Kaplan-parameter-scaling-law-curve.png';
import Kaplan_ablation_lstm from '../../../assets/image/Kaplan-ablation-LSTM.png';
import Kaplan_parameter_scaling_law_generalization from '../../../assets/image/Kaplan-parameter-scaling-law-generalization.png';
import Kaplan_ablation_depth_to_generalization from '../../../assets/image/Kaplan-ablation-depth-to-generalization.png';
import Kaplan_dataset_size_scaling_law from '../../../assets/image/Kaplan-dataset-size-scaling-law.png';
import Kaplan_compute_scaling_law from '../../../assets/image/Kaplan-compute-scaling-law.png';
import Kaplan_sample_efficiency_vs_model_size from '../../../assets/image/Kaplan-sample-efficiency-vs-model-size.png';
import Kaplan_critical_batch_size_relation from '../../../assets/image/Kaplan-critical-batch-size-relation.png';
import Kaplan_critical_batch_size_vs_performance from '../../../assets/image/Kaplan-critical-batch-size-vs-performance.png';
import Kaplan_loss_vs_model_size_and_training_steps from '../../../assets/image/Kaplan-loss-vs-model-size-and-training-steps.png';
import Kaplan_performance_vs_compute_budget_and_steps from '../../../assets/image/Kaplan-performance-vs-compute-budget-and-steps.png';
import Kaplan_early_stop_resultes from '../../../assets/image/Kaplan-early-stop-resultes.png';
import Kaplan_loss_vs_optimal_compute from '../../../assets/image/Kaplan-loss-vs-optimal-compute.png';
import Kaplan_optimal_model_size_given_compute from '../../../assets/image/Kaplan-optimal-model-size-given-compute.png';
import Kaplan_suboptimal_model_efficiency from '../../../assets/image/Kaplan-suboptimal-model-efficiency.png';
import Kaplan_optimal_step_size_vs_compute from '../../../assets/image/Kaplan-optimal-step-size-vs-compute.png';
import Kaplan_learning_rate_schedule_results from '../../../assets/image/Kaplan-learning-rate-schedule-results.png';

## Introduction

OpenAI 提出了针对decoder only LLM的scaling law [@Kaplan].
...
1. 当数据集充分大时，损失与模型参数大小以及更新步数 $S$ 的关系如下
...
1. 给定算力 $C$ 且无其他限制时，模型参数，数据及大小，batch size 和更新参数与算力之间的关系如下
...
这里 $d$ 是 hidden size, $n$ 是 layer 个数，$d_{ff}$ 是 MLP 的 hidden size, 这里我们 假设 $d_{ff}=4d$. 计算时我们丢掉了 bias 以及 LayerNorm 的参数量。具体计算过程见 [LLM parameter analysis](/p/llm-parameter-computation/)
...
具体计算过程见 [LLM FLOPs analysis](/p/llm-flops-computation/)。也就是说，对于参数量为 $N$ 的 transformer model, 每个 token 所需要的 FLOPs 为 $C\approx 6N$
...
<Image src={Kaplan_model_shape_ablation} alt="Ablation study on model shape" />
...
<Image src={Kaplan_ablation_model_size} alt="Ablation study on model size" />
...
<Image src={Kaplan_parameter_scaling_law_curve} alt="Scaling law with respect to parameters" />

### Comparing to LSTMs and Universal Transformers

作者比较了 LSTM 和 Transformer 结构的损失，结果如下图所示

<Image src={Kaplan_ablation_lstm} alt="Ablation study on LSTM" />
...
<Image src={Kaplan_parameter_scaling_law_generalization} alt="Generalization performance" />

结果发现，模型在其他数据集上的泛化性很好。并且，**模型的泛化性能仅与训练阶段的表现相关（validation loss），而与训练阶段（是否收敛）无关**。

作者还评估了 model depth 对模型泛化性的影响，结果如下图所示

<Image src={Kaplan_ablation_depth_to_generalization} alt="Ablation study on depth" />
...
<Image src={Kaplan_dataset_size_scaling_law} alt="Scaling law with respect to dataset" />
...
<Image src={Kaplan_compute_scaling_law} alt="Scaling law with respect to compute" />

作者进一步探究了 sample efficiency 与 model size 之间的关系，实验结果如下图所示

<Image src={Kaplan_sample_efficiency_vs_model_size} alt="Sample efficiency with respect to model size" />
...
<Image src={Kaplan_critical_batch_size_relation} alt="Critical batch size relation" />
...
<Image src={Kaplan_critical_batch_size_vs_performance} alt="Critical batch size vs performance" />
...
给定一个 target loss $L$, 当 batch size $B \gg  B_{crit}$ 时，作者定义最小训练步数为

$$
S_{\min}(S) := \frac{S}{1+B_{crit}(L)/B}
$$

给定 target loss $L$ 和 model size $N$, 当 batch size $B \ll  B_{crit}$ 时，作者定义最小算力为
...
<Image src={Kaplan_loss_vs_model_size_and_training_steps} alt="Loss vs model size and training steps" />

作者还使用了不同的可视化方式，如下图所示

<Image src={Kaplan_performance_vs_compute_budget_and_steps} alt="Performance vs compute budget and steps" />
...
<Image src={Kaplan_early_stop_resultes} alt="Early stop results" />
...
<Image src={Kaplan_loss_vs_optimal_compute} alt="Loss vs optimal compute" />

作者发现，相比于 loss 与算力 $C$ 之间的关系，使用 $C_{\min}$ 进行拟合效果更好。

接下来，作者基于 $L(C_{\min})$ 进一步探究了给定算力如何决定最优的 model size $N(C_{\min})$. 其实验结果如下图所示

<Image src={Kaplan_optimal_model_size_given_compute} alt="Optimal model size given compute" />
...
<Image src={Kaplan_suboptimal_model_efficiency} alt="Suboptimal model efficiency" />
...
<Image src={Kaplan_optimal_step_size_vs_compute} alt="Optimal step size vs compute" />
...
<Image src={Kaplan_learning_rate_schedule_results} alt="Learning rate schedule results" />
