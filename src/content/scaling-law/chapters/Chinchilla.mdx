...
已有工作如 [Kaplan scaling law](/p/kaplan-scaling-law/) 已经发现模型参数和大语言模型表现之间的关系，一个结论就是计算最优并不代表达到最优的 loss. 在本文中，作者也有相同结论，但是作者认为大模型应该使用比 Kaplan scaling law 推荐的更多的 training token. 基于这个发现，作者训练了 Chinchilla [@Chinchilla], 一个 70B 的 LLM, Chinchilla 相比 Gopher 表现有了大幅度的提升。
