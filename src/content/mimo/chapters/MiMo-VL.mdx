...
<Image src={architecture} alt="Architecture of MiMo-VL 7B" />
...
![Pretraining of MiMo-VL 7B](/posts/notes-on-mimo-vl/pretraining.png)
...
![RL Training](/posts/notes-on-mimo-vl/RL_training.png)
...
![Performance of MiMo-VL-7B](/posts/notes-on-gemma3/performance.png)
...
![Ablation of on-policy GRPO](/posts/notes-on-mimo-vl/on-policy-GRPO-ablation.png)

最后，作者探讨了一下将不同任务放在一起训练导致的互相干扰问题，这个在MiMo里也提到过，核心问题就是有的任务是short CoT的，有的任务是Long CoT的，如何让模型根据任务来选择不同的reasoning length是一个需要探讨的问题，这个问题在[VC-PPO](/p/notes-on-vc-ppo/)里进行了初步的探讨。
