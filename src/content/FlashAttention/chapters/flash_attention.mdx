...
import flashattention_gpu_hierarchy from '../../../assets/image/flashattention-GPU-hierarchy.png';
import flashattention_standard_attention_implementation from '../../../assets/image/flashattention-standard-attention-implementation.png';
import flashattention_standard_attention_backward_pass from '../../../assets/image/flashattention-standard-attention-backward-pass.png';
import flashattention_algorithm from '../../../assets/image/flashattention-algorithm.png';
import flashattention_forward_pass_algorithm from '../../../assets/image/flashattention-forward-pass-algorithm.png';
import flashattention_backward_pass from '../../../assets/image/flashattention-backward-pass.png';
import flashattention_block_sparse_algorithm from '../../../assets/image/flashattention-block-sparse-algorithm.png';
import flashattention_block_sparse_efficiency from '../../../assets/image/flashattention-block-sparse-efficiency.png';
import flashattention_ablation_block_size from '../../../assets/image/flashattention-ablation-block-size.png';
...
2. 从空间复杂度上入手，比如使用 [GQA](/p/notes-on-gqa/), [MQA](/p/notes-on-mqa/) 等减少内存的访问开销

本文提出的 flash attention [@FlashAttention] 就属于降低空间复杂度的一种做法。作者认为，我们应该设计一种 **IO-aware** 的 attention 算法，来减少 attention 计算式的内存访问开销，进而提高 attention 的计算效率。
...
<Image src={flashattention_gpu_hierarchy} alt="Memory Hierarchy of GPU" />
...
<Image src={flashattention_standard_attention_implementation} alt="Algorithm 0: Standard Attention Implementation" />
...
<Image src={flashattention_standard_attention_backward_pass} alt="Standard attention backward pass" />
...
因此，如果我们额外记录 $m(x)$ 以及 $\ell(x)$ 这两个量，那么我们可以每次仅计算 softmax 的一个 block. 具体细节见[softmax](/p/notes-on-softmax/).
...
<Image src={flashattention_algorithm} alt="FlashAttention algorithm" />
...
<Image src={flashattention_forward_pass_algorithm} alt="FlashAttention forward pass" />
...
注意到 $P_{i:}=\mathrm{softmax}(s_{i:})$, 且 $y=\mathrm{softmax}(x)$ 的 Jacobian 是 $\mathrm{diag}(y)-yy^T$ (推导过程见 [softmax](/p/notes-on-softmax/)), 我们有
...
<Image src={flashattention_backward_pass} alt="FlashAttention backward pass" />
...
<Image src={flashattention_block_sparse_algorithm} alt="Block-sparse FlashAttention forward pass" />
...
<Image src={flashattention_block_sparse_efficiency} alt="Efficiency of block-sparse FlashAttention" />
...
<Image src={flashattention_ablation_block_size} alt="Ablation on block size" />
