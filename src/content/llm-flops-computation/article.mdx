...
description: 我们介绍如何计算基于 transformer 架构的 LLM 的 FLOPs, 计算完成之后，我们可以推导出算力 $C$ 与模型参数量 $N$，数据集大小
  $D$ 之间的关系，即 $C\approx 6ND$.
authors:
- name: Mao Song
  url: https://maosong.website/
published: 2025-10-15 16:33:39+0800
...
- Tutorial
showPdf: false
seoThumbImage: /posts/llm-flops-computation/LLM-FLOPs-flops_distribution.png
---
...
我们计算训练阶段的总 FLOPs, 记为 $C$,  [Kaplan scaling law](/p/kaplan-scaling-law/) 用 PF-days 作为单位，$1\text{ PF-Days}=10^{15}\times 24\times 3600\ FLOPs$. 训练阶段包括前向阶段 (forward pass) 和反向传播阶段 (backward pass). 因此
...
MoE 是针对 Dense FFN 的一个改进，介绍见 [MoE](/p/moe-tutorial/), 我们假设一共有 $e$ 个路由专家，其中激活 $k$ 个。
...
首先，在 [LLM parameter analysis](/p/llm-parameter-computation/) 中，我们已经给出了 LLM 参数量 $N$ （基于 [Qwen3](/p/notes-on-qwen3/)）的计算结果
...
我们忽略关于 $d$ 的一阶项，并且我们假设 $|V|  \ll  12nd$, 则最终模型参数量可以近似为
...
这里我们利用了前面的 $|V|  \ll  12nd$ 假设，为了简便我们还舍弃了 $4s/d$.
...
接下来我们定量分析一些模型的 FLOPs. 我们基于 [Chinchilla scaling law](/p/chinchilla-scaling-law/) 给出的实验配置 (Table A9), 我们筛掉 `kv_size * n_heads != d_model` 的配置，$|V|=32,000$.
...
![FLOPs distribution against model size](../../assets/image/LLM-FLOPs-flops_distribution.png)

可以看到，当模型越来越大，FFN 层的算力占比越来越高，这也是为什么后来采用 MoE 架构的一个原因。

接下来，我们看一下模型 FLOPs 分布情况随上下文长度变化的情况

![FLOPs distribution aginst context length](../../assets/image/LLM-FLOPs-flops_vs_context.png)
