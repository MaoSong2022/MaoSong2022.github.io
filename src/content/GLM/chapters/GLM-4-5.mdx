...
description: 智谱 AI 提出了 GLM4.5, 包含 GLM4.5 和 GLM-4.5-Air,两个 MoE LLM. 模型大小分别为 355B-A22B
  和 106B-A12B, GLM4.5 主要关注 agentic, reasoning 以及 coding 三个领域。
authors:
- name: Mao Song
  url: https://maosong.website/
published: 2025-08-13 12:27:48+0800
modified: 2025-08-13 12:27:48+0800
tags:
- Zhipu
- Reasoning
categories:
- LLM
showPdf: false
seoThumbImage: /posts/notes-on-glm-4-5/GLM4.5-agent-coding-performance.png
---
import Image from '../../../components/Image.astro';
import glm4_5_pre_training from '../../../assets/image/GLM4_5-pre-training.png';
import glm4_5_post_training_framework from '../../../assets/image/GLM4.5-post-training-framework.png';
import glm4_5_curriculum_rl from '../../../assets/image/GLM4.5-curriculum-RL.png';
import glm4_5_multi_stage_context_extension from '../../../assets/image/GLM4.5-multi-stage-context-extension.png';
import glm4_5_code_science_rl from '../../../assets/image/GLM4.5-code-science-RL.png';
import glm4_5_interaction_turns from '../../../assets/image/GLM4.5-interaction-turns.png';
import glm4_5_average_performance from '../../../assets/image/GLM4.5-average_performance.png';
import glm4_5_agent_coding_performance from '../../../assets/image/GLM4.5-agent-coding-performance.png';
...
2. 与 [Kimi-k2](/p/notes-on-kimi-k2/) 和 [DeepSeek-V3](/p/notes-on-deepseek-v3/) 相比，作者降低了 head dimension, 提升了 number of layers. 作者认为更深的模型更有利于提高模型的 Reasoning 表现
3. attention 上，作者使用了 [GQA](/p/notes-on-gqa/), 对于 #RoPE, 作者使用了 partial RoPE, 只旋转每个 token 的前半部分， 作者还将 attention heads 的个数增加到了 2.5 倍，作者发现增加 attention heads 可以提高模型的 Reasoning 表现
4. 作者还使用了 [QK-Norm](/p/notes-on-qk-norm/) 来防止 attention logits 爆炸
5. 作者还使用了一个 MoE layer 作为 MTP layer 来支持 speculative decoding.

模型与 [DeepSeek-V3](/p/notes-on-deepseek-v3/) 和 Kimi-k2 的对比如下
...
<Image src={glm4_5_pre_training} alt="Pre-training recipe of GLM4.5" />
...
训练时，与 Kimi-k2 一样，作者使用了 [Muon](/p/notes-on-moonlight/) 作为优化器。作者使用了 cosine decay schedule. batch size 从 16M token 到 64M token.
...
<Image src={glm4_5_post_training_framework} alt="Post-training of GLM4.5" />
...
这个阶段使用了 GRPO 算法进行训练，与 [DAPO](/p/notes-on-dapo/) 一样，作者去除了损失函数中的 KL divergence。

首先，作者探究了课程学习对模型表现的影响，结果发现，课程学习可以有效提高模型的性能。因此，作者构建了一个 2 阶段的课程学习框架。实验结果如下图所示

<Image src={glm4_5_curriculum_rl} alt="Performance of curriculum RL" />
...
<Image src={glm4_5_multi_stage_context_extension} alt="Ablation on progressive context extension" />
...
<Image src={glm4_5_code_science_rl} alt="Ablation on Code and Science RL" />
...
> Recall
> 在 [GLM-4.V-Thinking](/p/notes-on-glm-4.1v-thinking/) 中，作者认为应该在 cold-start SFT 阶段让模型学会格式要求，而不是在 RL 阶段加入 format reward
...
> Recall
> [Seed1.5-VL](/p/notes-on-seed1.5-vl/) 和 LlaMA3.2 都提到了使用 multi-round SFT-RL 的形式来提高模型的表现

作者还发现，随着交互轮数的提升，模型的表现也有相应提升。实验结果如下图所示

<Image src={glm4_5_interaction_turns} alt="Interaction turn scaling" />
...
<Image src={glm4_5_average_performance} alt="Average performance on ARC benchmarks" />
...
<Image src={glm4_5_agent_coding_performance} alt="Comparison of GLM4.5 aginst other models" />
