...
description: 快手提出了 Keye-VL 1.5, 一个强调 reasoning, video understanding 的 8B 多模态大模型。作者提出了
  slow-fast video encoding strategy 来提高模型的视频理解能力，作者通过在预训练和后训练提高了模型的长上下文能力和 reasoning
  能力
authors:
- name: Mao Song
...
import keye_vl_1_5_post_training_pipeline from '../../../assets/image/Keye-VL-1-5-post-training-pipeline.png';
import keye_vl_1_5_longcot_data_generation_pipeline from '../../../assets/image/Keye-VL-1-5-LongCoT-data-generation-pipeline.png';
import keye_vl_1_5_performance from '../../../assets/image/Keye-VL-1-5-Performance.png';
import keye_vl_1_5_ablation_training_stage from '../../../assets/image/Keye-VL-1-5-ablation-training-stage.png';
import keye_vl_1_5_ablation_model_merging from '../../../assets/image/Keye-VL-1-5-ablation-model-merging.png';
import keye_vl_1_5_ablation_alignment_rl from '../../../assets/image/Keye-VL-1-5-ablation-alignment-RL.png';
import keye_vl_1_5_ablation_hint from '../../../assets/image/Keye-VL-1-5-ablation-hint.png';
import keye_vl_1_5_ablation_rejection_sampling from '../../../assets/image/Keye-VL-1-5-ablation-rejection-sampling.png';
...
Keye-VL 1.5 的架构与 [Keye-VL](/p/notes-on-keye-vl/) 一致。

Keye-VL 1.5 主要做出的改进点为针对视频的 encoding 方式

作者回顾了已有 MLLM 处理视频的方式，比如 [Qwen2.5-VL](/p/notes-on-qwen2.5-vl/) 使用 3D convolution 来 merge 相邻的两帧，[Seed1.5-VL](/p/notes-on-seed1.5-vl/) 采用了 Dynamic Frame-Resolution Sampling 技巧，来根据 budget 和处理的任务来动态调整采样率 (frame) 和每一帧的图片精度 (resolution)。
...
<Image src={keye_vl_1_5_post_training_pipeline} alt="Post-Training Pipeline" />
...
<Image src={keye_vl_1_5_longcot_data_generation_pipeline} alt="LongCoT data generation pipeline" />
...
对于通用的 RLVR 训练，作者使用了 [GSPO](/p/notes-on-gspo/) 算法来进行训练。
...
为了进一步提高模型的表现，作者采用了一个和 [Seed1.5-VL](/p/notes-on-seed1.5-vl/) 一样的迭代式训练策略，即反复进行 SFT 和 RL 来降低训练成本，提高训练效率。
...
<Image src={keye_vl_1_5_performance} alt="Performance of Keye-VL 1.5" />

接下来作者进行了消融实验。

首先是不同训练阶段对模型表现的影响，如下图所示

<Image src={keye_vl_1_5_ablation_training_stage} alt="Ablation study on training strategy" />
...
<Image src={keye_vl_1_5_ablation_model_merging} alt="Ablation study on model merging" />
...
<Image src={keye_vl_1_5_ablation_alignment_rl} alt="Ablation on alignment RL" />

实验结果说明，alignment RL 可以在保持模型 reasoning 能力的同时提高模型的指令跟随能力

作者还探究了 hint 对模型表现的影响，使用不同 level hint 进行训练对模型表现影响的结果如下图所示

<Image src={keye_vl_1_5_ablation_hint} alt="Ablation on hint level" />

实验结果显示，使用 high level 的 hint 可以有效提高模型输出的正确率。

最后作者探究了 rejection sampling 对模型表现的影响，实验结果如下图所示

<Image src={keye_vl_1_5_ablation_rejection_sampling} alt="Ablation on rejection sampling" />
