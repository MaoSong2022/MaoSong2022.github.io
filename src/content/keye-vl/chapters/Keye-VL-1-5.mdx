...
快手在 25年9月 提出了 Keye-VL 1.5 [@Keye-VL1.5], 一个强调 reasoning, video understanding 的 8B 多模态大模型。
Keye-VL-1.5 相比于 [Keye-VL](#keye-vl) 主要做了三点改进：
...
Keye-VL 1.5 的架构与 [Keye-VL](#keye-vl) 一致。

Keye-VL 1.5 主要做出的改进点为针对视频的 encoding 方式

如 [Qwen2.5-VL](https://maosong.website/blog/qwen-vl-series/#qwen25-vl) 使用 3D convolution 来 merge 相邻的两帧，Seed1.5-VL [@Seed1.5-VL] 采用了 Dynamic Frame-Resolution Sampling 技巧，来根据 budget 和处理的任务来动态调整采样率 (frame) 和每一帧的图片精度 (resolution).

作者认为这些方法比较难进行泛化，因此，作者在本文中就提出了 **SlowFast video encoding strategy**:
...
<Image
  src={keye_vl_1_5_longcot_data_generation_pipeline}
...
<Image
  src={keye_vl_1_5_ablation_training_stage}
...
<Image
  src={keye_vl_1_5_ablation_model_merging}
...
<Image
  src={keye_vl_1_5_ablation_rejection_sampling}
