...
description: Tokenizer 总结与 BPE tokenizer 的高效实现
authors:
- name: Mao Song
  url: https://maosong.website/
published: 2025-05-24 19:56:34+0800
tags:
- Tokenizer
categories:
- LLM
...
import tokenization_pipeline from '../../assets/image/tokenization_pipeline.png';
import bpe_algorithm from '../../assets/image/bpe_algorithm.png';
import unigram from '../../assets/image/unigram.png';
import Embeds from '../../assets/embeds/tokenizer-comparison-with-classification.html';

## Introduction

在自然语言处理中，tokenizer 的作用是将一个文本序列通过一个字典转化为一个 token id 的序列。
回顾图片分类 (image classification) 任务，模型在预测的时候，实际上预测的是类别对应的 id，而不是类别本身。
tokenizer 做的事情就是提供一个类似于从类别到对应 id 的字典。
...
<Image src={tokenization_pipeline} alt="Tokenization pipeline" />
...
| subword-level | Leverages common prefixed and suffixes; learned from data | risk of oov, through less than word level | 
| character level | small chance of OOV; robust to casing and misspellings | makes computation slower; embeddings not interpretable | 
...
BPE [@BPE]，即 byte pair tokenizer 的原理也是类似的，对于出现频率比较高的byte pair或者character pair, 我们会使用一个新的token来表示这个pair，这样就压缩了sequence的长度。
...
<Image src={bpe_algorithm} alt="BPE algorithm" />
...
WordPiece [@WordPiece] 是Google在预训练BERT时采用的tokenizer，WordPiece的基本思想和BPE差不多，都是从一个较小的vocab开始的。
...
Unigram [@Unigram] 也是由Google提出来的tokenizer，与BPE和wordpiece不同，unigram从一个非常大的vocab开始，然后merge token来降低vocab的size，直到达到指定大小。初始的vocab可以基于BPE算法或者使用prefix subword来构建。并且，初始vocab还包含所有的base characters来保证所有的word都可以被tokenize。

算法的描述如下:

<Image src={unigram} alt="Unigram algorithm" />
...
[SentencePiece](https://github.com/google/sentencepiece)是google开源的一个无监督的text tokenizer [@SentencePiece]，其实现了BPE和unigram两种算法，SentencePiece还是一个语言无关的tokenizer，使其更适合多语种大语言模型的开发。
