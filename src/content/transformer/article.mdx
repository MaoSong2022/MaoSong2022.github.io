...
published: 2025-06-29 11:40:39+0800
modified: 2025-06-29 11:40:39+0800
tags:
- cs336
- Transformer
categories:
...
import transformer_architecture from '../../assets/image/transformer_architecture.png';
...
Bethune et.al 2025 [@bethune2025scalinglawsforgettingfinetuning] 发现在 finetuning 过程中加入 $1\%$ 的预训练数据就可以避免灾难性遗忘。

## 架构

Transformer 实现

我们采用 top-down 的形式构建 transformer [@vaswani2017attention] 的代码。

<Image src={transformer_architecture} alt="Transformer architecture" />
...
苏剑林老师提出了 [Position Encoding](/p/notes-on-position-encoding/)，现在已经被广泛使用
...
1. Encoder-decoder: text to text, T5, mT5, ByT5
2. Encoder only: BERT, DistilBERT, RoBERTa
3. Decoder only: GPT, LLaMA, Qwen, DeepSeek
